...
resource "google_iap_tunnel_instance_iam_policy" "healthz" {
  project     = "${local.project_id}"
  provider    = "google-beta"
  instance    = "${google_compute_instance.healthz.name}"
  zone        = "${var.zone}"
...
resource "google_service_account_iam_member" "tunnel_user" {
  ...
  service_account_id = "${google_service_account.shared_service.name}"
